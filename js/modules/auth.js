...
export async function salvarNomeArcano() {
  const inputNome    = document.getElementById("nomeArcanoInput");
  const inputProva   = document.getElementById("provaInput");
  const inputData    = document.getElementById("dataProvaInput");

  const nome     = inputNome?.value?.trim() || "";
  const prova    = inputProva?.value?.trim() || "";
  const dataProva = inputData?.value?.trim() || "";

  // exige nome, os outros podem ser vazios
  if (!nome) {
    if (inputNome) {
      inputNome.style.borderColor = "#c4554c";
  ...
  // Recarrega para disparar init completo com o novo nome
